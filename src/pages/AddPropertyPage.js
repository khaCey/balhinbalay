...
function AddPropertyPage() {
  ...
  if (
    isEdit &&
    (loading || !initialListing || initialListing.ownerId !== user.id)
  ) {
    return (
      <div className="bb-empty">
        <h1>{loading ? 'Loading your listing…' : 'Listing unavailable'}</h1>
        <p>
          {loading
            ? 'Please wait.'
            : 'This listing could not be opened for editing.'}
        </p>
      </div>
    );
  }
